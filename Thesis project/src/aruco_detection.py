...
# --- Find which grid cell marker is in ---
def get_cell_coords(cx, cy, frame_shape):
    h, w, _ = frame_shape
    cell_x, cell_y = int((cx / w) * NUM_CELLS), int((cy / h) * NUM_CELLS)
    return (cell_x, cell_y)
# --- Convert grid coordinates to pixel center --- ( added newly on 31/10/2025)
#when using this comment out the previous function get_cell_coords and use this function instead
...
stable_text = []
text_history = deque(maxlen=10)  # keeps last few frames’ text to smooth flicker
# --- Path storage ---
#newly added on 31/10/2025
...
while True:
    ...
    current_text = []
    #comment when using code written on 31/10/2025
    stable_detections = {}  # marker_id → (color_name, grid_x, grid_y)
    ...
    if ids is not None:
        ...
        for i, corner in enumerate(corners):
            ...
            #comment this when using code written on 31/10/2025 
            # i.e when using path visualization algo
            ...
            if id_val == REFERENCE_ID:
                ...
                #ise bhi comment kar dena
                stable_detections[id_val] = ("reference", cell_x, cell_y)
            ...
            # Color-assigned cubes
                ...
                cv.circle(frame, (cx, cy), 5, color, -1)
                #ise bhi comment kar dena
                stable_detections[id_val] = (color_name, cell_x, cell_y)
                current_text.append(f"ID {id_val}: {color_name} ({cell_x}, {cell_y})")

    # --- Flicker suppression logic ---
    if current_text:
    ...
    # --- Path Planning Trigger (once after stable detections) ---
    if len(text_history) == text_history.maxlen:  # detections stable for a few frames
        # Collect cube coordinates
        red_cubes, green_cubes, blue_cubes = [], [], []
        reference_cell = None

        for t in stable_text:
            if "Reference" in t:
                ref_part = t.split("(")[-1].strip(")")
                gx, gy = map(int, ref_part.split(","))
        ...
        if reference_cell:
            ...
            for color, cubes in [("🟥 RED", red_cubes), ("🟩 GREEN", green_cubes), ("🟦 BLUE", blue_cubes)]:
                if cubes:
                    path_greedy, dist_greedy = greedy_path(reference_cell, cubes)
                    path_tsp, dist_tsp = tsp_bruteforce(reference_cell, cubes)
                    print(f"\n{color} cubes: {cubes}")
                    print(f"Greedy Path: {path_greedy} | Distance: {dist_greedy}")
                    print(f"TSP Path: {path_tsp} | Distance: {dist_tsp}")
        text_history.clear()  # prevent re-printing on next frame
